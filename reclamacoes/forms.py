...
class ReclamacoesForm(forms.ModelForm):
    cpf = forms.CharField(
        ...
        widget=forms.TextInput(attrs={'id': 'id_cpf'}),
        required=False
    )
    acompanhar_reclamacao = forms.BooleanField(
        label='DESEJO ACOMPANHAR MINHA RECLAMAÇÃO.',
        required=False,
        widget=forms.CheckboxInput(attrs={'id': 'id_acompanhar_reclamacao'})
    )

    class Meta:
        model = Reclamacoes
        fields = [
            'nome', 'tipo_reclamacao', 'observacao',
            'cep', 'rua', 'numero_casa', 'complemento',
            'referencia', 'telefone', 'acompanhar_reclamacao', 'cpf'
        ]
        labels = {
            'acompanhar_reclamacao': 'OPCIONAL',
            'telefone': 'TELEFONE',
            ...
            'nome': 'NOME COMPLETO',
            'cpf': 'CPF',
            'observacao': 'DETALHES DA RECLAMAÇÃO',
    ...
    def clean(self):
        ...
        if acompanhar_reclamacao and not cpf:
            self.add_error('cpf',
                           'Para acompanhar sua reclamação o campo CPF é obrigatório')
